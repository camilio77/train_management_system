...
public class EmployeePanelView {
    ...
    public EmployeePanelView(AbstractUserWithPower employee) {
        ...
        btnLogout.addActionListener(e -> { frame.dispose(); new LoginView(); });
    ...
    private JPanel buildRouteTab() {
        ...
        btnAdd.addActionListener(e -> {
            try {
                List<Train> trenes = TrainService.getInstance().getAll();
                List<Station> estaciones = StationService.getInstance().getAll();
        ...
        btnEdit.addActionListener(e -> {
            ...
            if (row < 0) { JOptionPane.showMessageDialog(frame, "Selecciona una ruta."); return; }
        ...
        try {
            for (Route r : RouteService.getInstance().getAll())
                model.addRow(new Object[]{
                    ...
                    r.getDateOfLeaving(), r.getDateOfArrival()
                });
        } catch (Exception ex) { showError(ex); }
    }
    ...
    private JPanel buildWagonTab() {
        ...
        btnEdit.addActionListener(e -> {
            int row = table.getSelectedRow();
            if (row < 0) { JOptionPane.showMessageDialog(frame, "Selecciona un vagón."); return; }
