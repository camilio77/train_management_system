...
public class TicketService extends UnicastRemoteObject implements TicketInterface {
  ...
  public TicketService() throws RemoteException { super(); }

  // ── Compra principal: devuelve lista de tickets (uno por tramo) ──
  @Override
  public LinkedList<Ticket> buyTickets(String idUsuario, String estacionOrigen,
    ...
    try {
      // 1. Dijkstra: camino de estaciones
      ...
      // 2. Por cada tramo adyacente, buscar la ruta que lo cubre
      ...
      // 3. Precio total = precio por categoría × número de tramos
      ...
      // 4. Crear un ticket por cada tramo
      ...
      try {
        ...
        for (int idRuta : idRutas) {
          ...
          // Equipaje solo en el primer tramo
          if (primerTramo && pesoEquipaje > 0) {
            ...
            if (idVagonEq > 0) psEq.setInt(2, idVagonEq);
            else               psEq.setNull(2, Types.INTEGER);
  ...
  // Busca la ruta que conecta dos estaciones adyacentes
  ...
  private int assignSeat(Connection conn, int idVagon, String categoria)
    ...
    switch (categoria.toUpperCase()) {
      case "PREMIUM"   -> { inicio = 1;  fin = CAP_PREMIUM; }
      case "EJECUTIVA" -> { inicio = CAP_PREMIUM + 1;
        fin = CAP_PREMIUM + CAP_EJECUTIVA; }
      default          -> { inicio = CAP_PREMIUM + CAP_EJECUTIVA + 1;
    ...
    while (rs.next()) ocupados.add(rs.getInt(1));
    for (int a = inicio; a <= fin; a++)
      if (!ocupados.contains(a)) return a;
    return -1;
  }
