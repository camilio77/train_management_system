...
public class WagonService extends UnicastRemoteObject implements WagonInterface {
    ...
    protected WagonService() throws RemoteException { super(); }
    ...
    public LinkedList<Wagon> getByTrain(int idTren) throws SQLException {
        ...
        PreparedStatement stmt = DatabaseConnection.getConnection().prepareStatement("SELECT * FROM vagon WHERE id_tren=?");
        stmt.setInt(1, idTren);
        ResultSet rs = stmt.executeQuery();
        while (rs.next()) list.add(new Wagon(String.valueOf(rs.getInt("id_vagon"))));
        return list;
    }
    ...
    public void create(int idTren, String tipo, int capacidad) throws SQLException {
        PreparedStatement stmt = DatabaseConnection.getConnection().prepareStatement("INSERT INTO vagon (id_tren, tipo, capacidad) VALUES (?, ?, ?)");
        stmt.setInt(1, idTren); stmt.setString(2, tipo);
    ...
    public void update(int id, String tipo, int capacidad) throws SQLException {
        PreparedStatement stmt = DatabaseConnection.getConnection().prepareStatement("UPDATE vagon SET tipo=?, capacidad=? WHERE id_vagon=?");
        stmt.setString(1, tipo); stmt.setInt(2, capacidad); stmt.setInt(3, id);
        stmt.executeUpdate();
    }

    @Override
    public void delete(int id) throws SQLException {
        PreparedStatement stmt = DatabaseConnection.getConnection().prepareStatement("DELETE FROM vagon WHERE id_vagon=?");
        stmt.setInt(1, id);
        stmt.executeUpdate();
    }
}